...
for (i in 1:30) {
  fangraphsPitching30d <-
    as.data.frame(rbind(fangraphsPitching30d,fangraphsP[[i]]))
}

# Rename columns
colnames(fangraphsPitching30d) <-
  c(
...
xPitch = sqldf(
  "SELECT a.Name, a.IP, a.SV, a.HLD, a.FBv,a.K_, a.BB_,
...
############### xPected K% ########################
xK_.fit = lm(
  K_ ~ IP + SwStr_ + OSwing_ + ZSwing_ + Swing_ + OContact_ + ZContact_ +
    Zone_ + FBv + Pull_ + FBvDIFF + K_DIFF + BB_DIFF + SwStr_DIFF + OSwing_DIFF +
    ZSwing_DIFF + Swing_DIFF + OContact_DIFF + ZContact_DIFF + FB_DIFF + Zone_DIFF +
    Hard_DIFF + Pull_DIFF, data = xPitch
)
summary(xK_.fit)
xPitch = cbind(
  xPitch,'xK_' = round(xK_.fit$fitted.values,2), 'GapxK_' = round(xK_.fit$residuals,2)
)

############### xPected BB% ########################
xBB_.fit = lm(
  BB_ ~ IP + SwStr_ + OSwing_ + ZSwing_ + Swing_ + OContact_ + ZContact_ +
    Zone_ + FBv + Pull_ + FBvDIFF + K_DIFF + BB_DIFF + SwStr_DIFF + OSwing_DIFF +
    ZSwing_DIFF + Swing_DIFF + OContact_DIFF + ZContact_DIFF + FB_DIFF + Zone_DIFF +
    Hard_DIFF + Pull_DIFF, data = xPitch
)
summary(xBB_.fit)
xPitch = cbind(
  xPitch,'xBB_' = round(xBB_.fit$fitted.values,2), 'GapxBB_' = round(xBB_.fit$residuals,2)
)

############### xPected aFIP #######################
xaFIP.fit = lm(aFIP ~ IP + xK_ + Hard_ + Pull_ + ERA + kwERA + SIERA + K_DIFF +
                 Pull_DIFF, data = xPitch)
summary(xaFIP.fit)
xPitch = cbind(
  xPitch,'xaFIP' = round(xaFIP.fit$fitted.values,2), 'GapxaFIP' = round(xaFIP.fit$residuals,2)
)
xPitching <-
  subset(
    xPitch, xaFIP < 5, select = c(
      Name,IP,SV,HLD,FBv,K_,xK_,GapxK_,BB_,xBB_,GapxBB_,HR_BB,SwStr_,SIERA,ERA,kwERA,FIP,xFIP,aFIP,xaFIP,GapxaFIP,FBvDIFF
    )
  )
